...
class VirtualScroller {
	...
	_sliceListData() {
		this.options.slicedListData = this.options.listData.slice(
			this.options.viewStartIndex,
			this.options.viewStartIndex + this.options.viewRenderCount
		)
		return this.options.slicedListData
	}
	...
	_insertHtml(sliceListData) {
		const vsListwrapperElement = this.containerElement.getElementsByClassName(`virtualscroller-listwrapper`)[0]
		const htmlString = sliceListData
			.map((item, index) => {

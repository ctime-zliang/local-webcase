...
;( function(window, undefined){
	...
	var eventDataArr = [
		...
		'onabort','onbeforeunload','onerror','onload','onmove','onresize','onscroll','onstop','onunload',
		'onblur','onchange','onfocus','onreset','onsubmit',
		'onbounce','onfinish','onstart',
		'onbeforecopy','onbeforecut','onbeforeeditfocus','onbeforepaste','onbeforeupdate','oncontextmenu','oncopy','oncut','ondrag','ondragdrop','ondragend','ondragenter','ondragleave','ondragover','ondragstart','ondrop','onlosecapture','onpaste','onselect','onselectstart',
		'onafterupdate','oncellchange','ondataavailable','ondatasetchanged','ondatasetcomplete','onerrorupdate','onrowenter','onrowexit','onrowsdelete','onrowsinserted',
		'onafterprint','onbeforeprint','onfilterchange','onhelp','onpropertychange','onreadystatechange'
	]; 
	...
	var qzql = {
		...
		'tap' : function( obj ){
			...
			obj['element'].addEventListener( 'touchstart', function( e ){
				...
				startClient['clientX'] = touches.clientX,
				startClient['clientY'] = touches.clientY;
			}, false );
			obj['element'].addEventListener( 'touchend', function( e ){
		...
		'doubleTap' : function( obj ){
			...
			obj['element'].addEventListener('touchend', function( e ){
				...
				if( Math.abs(startClient['clientX'] - endTx ) < trigger && Math.abs( startClient['clientX'] - endTx ) < trigger ){  //首先要确保能触发单次的 tap 事件
					if ( duration < interval ){  // 两次tap的间隔确保在500毫秒以内
						if ( lastTx !== null && Math.abs( lastTx - endTx ) < offset && Math.abs( lastTy - endTy ) < offset ) {  //本次的tap位置和上一次的tap的位置允许一定范围内的误差
							firstTouchEnd = true;
							lastTx = lastTy = null;
		...
		'longTap' : function( obj ){
			...
			obj['element'].addEventListener('touchmove', function( e ){
				var touches = e.touches[0];
		...
		'swipe' : function( obj, innerCallBackFn ){
			...
			obj['element'].addEventListener('touchstart', function( e ){
				...
				startClient['clientX'] = touches.clientX,
				startClient['clientY'] = touches.clientY;
				obj['element'].BuiltInGetLastClientX = startClient['clientX'],
				obj['element'].BuiltInGetLastClientY = startClient['clientY'];
		...
		'swipeUp' : function( obj ){
			...
			qzql.swipe(obj, function(obj, startClient, config){
				obj['element'].addEventListener('touchmove', function( e ){
					...
					if( touches.clientY - obj['element'].BuiltInGetLastClientY > 0 ){ //to bottom
		...
		'swipeDown' : function( obj ){
			...
			qzql.swipe(obj, function(obj, startClient, config){
				obj['element'].addEventListener('touchmove', function( e ){
					...
					if( touches.clientY - obj['element'].BuiltInGetLastClientY < 0 ){ //to top
						obj['element'].BuiltInGetLastClientY = touches.clientY;
						if( !single ){
							if( ( Math.abs( touches.clientX - startClient['clientX'] ) < Math.abs( touches.clientY - startClient['clientY'] ) ) && ( Math.abs( touches.clientX - startClient['clientX'] ) < obj['relataxis'] ) ){
								obj['fnDuring'] && obj['fnDuring'].call(obj['element'], touches, startClient, -1);
		...
		'swipeLeft' : function( obj ){
			...
			qzql.swipe(obj, function(obj, startClient, config){
				obj['element'].addEventListener('touchmove', function( e ){
					...
					if( touches.clientX - obj['element'].BuiltInGetLastClientX > 0 ){ //to right
		...
		'swipeRight' : function( obj ){
			...
			qzql.swipe(obj, function(obj, startClient, config){
				obj['element'].addEventListener('touchmove', function( e ){
					...
					if( touches.clientX - obj['element'].BuiltInGetLastClientX < 0 ){ //to left
						obj['element'].BuiltInGetLastClientX = touches.clientX;
						if( !single ){
							if( ( Math.abs( touches.clientX - startClient['clientX'] ) > Math.abs( touches.clientY - startClient['clientY'] ) ) && ( Math.abs( touches.clientY - startClient['clientY'] ) < obj['relataxis'] ) ){
								obj['fnDuring'] && obj['fnDuring'].call(obj['element'], touches, startClient, -1);
		...
		'trim' : function(str, isAll){
			...
			if( isAll && isAll === true ){  //全局匹配,过滤所有空格
		...
		'ready' : function( callback ){
			if( ini.classOf( callback ) == 'function'){  //检测传参是否为function类型
				if( document.addEventListener ){  //DOM事件监听
		...
		'query' : function(selector, context){
			if( typeof selector == 'undefined' ){  //无传参时退出
				return; 
			}
			if( ini.classOf( selector ) == 'string' ){  //CSS选择器字符串
				var tmp;
				if( typeof context == 'undefined' ){  //"上下文"未定义
					tmp = qzql.qzql_FIND_NODE(selector, document);
				}else if( context && ( context.nodeType == 1 || context.nodeType == 9 ) ){  //"上下文"定义为节点类型
		...
		'eq' : function(selector, index){
			...
			if( ini.classOf( tmp ) == 'nodelist' || ini.classOf( tmp ) == 'htmlcollection' ){  //检测类型是否为节点类数组
		...
		'next' : function(selector, condition){
			...
			if( typeof condition == 'undefined' ){  //限制条件未定义
				return tmp.nextSibling;
			}else if( ini.classOf( condition ) == 'string' ){  //定义限制条件为字符串
				...
				findIndex : for( var j = 0; j < arrIndex.length; j++ ){
					if( arrIndex[j] > _index ){  //查询索引值大于参考节点的索引值的最小索引值所对应的节点
		...
		'prev': function(selector, condition){
			...
			if( typeof condition == 'undefined' ){  //限制条件未定义
				...
				findIndex : for ( var j = 0; j < arrIndex.length; j++ ){
					if( arrIndex[j] > _index ){  //获取索引值大于参考节点的索引值的最小索引值
						k = j;
						break findIndex;
					}
				}
			...
			function callBackFn(){
				...
				if( xhr.status == 200 ){
					...
					if( argsuments.returnAll ){
						argsuments.success.call(xhr, callBackResponse || {
							'body' : '',
							'header' : {}
						}, null);  //回调
					}else{
		...
		'serializeForm': function( form ){
			if( form.nodeType == 1 && form.tagName.toLowerCase() == 'form' ){
				var content = {},
					...
					checkbox_string = '';
				for( var i = 0; i < form_element_len; i++){ //循环遍历表单控件
		...
		'getScrollTop': function(){
			...
			if( document.documentElement ){
				documentScrollTop = document.documentElement.scrollTop;　　scrollTop = ( bodyScrollTop - documentScrollTop > 0 ) ? bodyScrollTop: documentScrollTop;　
			}　
			return scrollTop;
		},

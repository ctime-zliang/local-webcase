	...
	createReceive() {
		return this.receiveBuilder()
			.match(FetchSlackData_1.FetchSlackData, fetchData => {
				Promise.all([slack_api_1.list(this.state.channel, this.state.latest).then(response => response.messages), slack_api_1.user()])
	...
	mergeUser2Message(messages, user) {
		return messages.map(message => {
			const userinfo = message.user
				? user.members.find(member => member.id === message.user)
				: {
